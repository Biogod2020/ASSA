    ...
    if (toolName === 'run_shell_command' || toolName === 'shell') {
        ...
        if (cmd.includes('git commit')) {
            ...
            additionalContext = '### ASSA TRIGGER: GIT COMMIT DETECTED ###\n' +
                ...
                '派发以下任务给它：\"作为 ASSA Distiller，读取最新的 .memory/evolution_ledger.json 和最近的 git diff，执行深度根因分析，并将结构化的 YAML 规则块写入 .memory/patterns.md。\" ' +
            ...
            additionalContext = '### ASSA TRIGGER: GIT PUSH DETECTED ###\n' +
                ...
                '派发以下任务给它：\"作为 ASSA Syncer，审计本地 .memory/patterns.md 中成熟度高的模式，剥离项目特定标识后，推广同步到全局库 ~/.gemini/assa/LIBRARY/ 下的相应文件中。\" ' +
